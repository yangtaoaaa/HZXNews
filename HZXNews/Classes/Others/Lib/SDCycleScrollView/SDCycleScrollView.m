...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            // 何紫璇1.18修改
            NSString *isWiFiBtnOnStr = [RXUserDefaultTool getObjectForKey:@"isWiFiBtnOn"];
            if([isWiFiBtnOnStr isEqualToString:@"0"]){ // 不加载图片
...
- (void)scrollToPic:(NSInteger)index{
    CGPoint point = _mainView.contentOffset;
    point.x = index * _mainView.width;
